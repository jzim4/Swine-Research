...
makeGraph <- function(theFile) {
  ...
  if (difference<0) {
    ...
    fillDayStanding <-c(rep(0,times=length(fillDayTime)))
    fillDay <<- data.frame(Time=fillDayTime,Standing=fillDayStanding)
  ...
  workingData$Standing <- ave(workingData$Standing,workingData$Day, FUN=cumsum)
  #add group row
  ...
  return(ggplot(workingData,aes(x=Minutes,y=Standing,group=Day, color = Group)) + 
           geom_line() +
           geom_text_repel(aes(label=Label)) +
           theme(legend.position = "none") +
           scale_x_continuous(breaks=c(indexOfHours),labels=hours)+
           labs(title = chartTitle, x = "Time of Day", y = "Cumulative Standing Time", colour="")
  )
}
...
printFiles <- function() {
  ...
  pdf("/Users/samzimmer/Desktop/pigs/Visualizations/standing.pdf")
  print("Files loaded")
