...
theFile <<- as.data.frame(read_excel("/Users/samzimmer/Desktop/pigs/Data/file01.xlsx",n_max=1440*7))
makeGraph <- function(theFile) {
  ...
  if (difference<0) {
    ...
    fillDayMotionIndex <-c(rep(0,times=length(fillDayTime)))
    fillDay <<- data.frame(Time=fillDayTime,MotionIndex=fillDayMotionIndex)
  ...
  workingData$MotionIndex <- ave(workingData$MotionIndex,workingData$Day, FUN=cumsum)
  #add group row
  ...
  quantify <- workingData[,1:4]
  quantify <- spread(quantify, Day, MotionIndex)
  colnames(quantify)[3:8] <- c("Day2","Day3","Day4","Day5","Day6","Day7")
  quantify <- quantify[order(quantify$Minutes),]
  rownames(quantify) <- 1:nrow(quantify)
  
  #FITTING AWAKE TIME
  quantitiesAwake <- data.frame(PigID="",Day="",Time="",Intercept="",Slope="",R2="")
  for (minutes in 1:5){
    for (days in 2:7) {
      time <- 420+(minutes*60)
      fit <- lm(quantify[(1:time),(days+1)]~quantify[(1:time),2], data=quantify)
      newLine <- data.frame(PigID=info$ID[pigInfo],Day=days,Time=time,Intercept=coef(fit)[1], Slope=coef(fit)[2],R2=summary(fit)$r.squared)
      quantitiesAwake <- rbind(quantitiesAwake,newLine)
    }
  }
  quantitiesAwake <- quantitiesAwake[order(quantitiesAwake$PigID,quantitiesAwake$Day,quantitiesAwake$R2),]
  quantitiesAwake <- quantitiesAwake[2:nrow(quantitiesAwake),]
  bestFitAwake <- quantitiesAwake[seq(5,nrow(quantitiesAwake),5),]
  rownames(bestFitAwake) <- 1:nrow(bestFitAwake)
  
  #FITTING ASLEEP TIME
  quantitiesAsleep <- data.frame(PigID="",Day="",startTime="",endTime="",Intercept="",Slope="",R2="")
  for (minutes in 1:5){
    for (days in 2:7) {
      startTime <- bestFitAwake$Time[days-1]
      endTime <- 1080+(minutes*60)
      fit <- lm(quantify[(startTime:endTime),(days+1)]~quantify[(startTime:endTime),2], data=quantify)
      newLine <- data.frame(PigID=info$ID[pigInfo],Day=days,startTime=startTime,endTime=endTime,Intercept=coef(fit)[1], Slope=coef(fit)[2],R2=summary(fit)$r.squared)
      quantitiesAsleep <- rbind(quantitiesAsleep,newLine)
    }
  }
  quantitiesAsleep <- quantitiesAsleep[order(quantitiesAsleep$PigID,quantitiesAsleep$Day,quantitiesAsleep$R2),]
  quantitiesAsleep <- quantitiesAsleep[2:nrow(quantitiesAsleep),]
  bestFitAsleep <- quantitiesAsleep[seq(5,nrow(quantitiesAsleep),5),]
  rownames(bestFitAwake) <- bestFitAsleep$Day
  
  
#TO DO:
  #right now, it kind of makes a row of data that can hold the pigID, day, time, equation, and r2
  #I need to make it so that it will try different times; I will probably do this by putting a for loop outside of the current for loop
  ...
  ggplot(workingData,aes(x=Minutes,y=MotionIndex,group=Day, color = Group)) + 
    geom_line() +
    geom_text_repel(aes(label=Label)) +
    theme(legend.position = "none") +
    scale_x_continuous(breaks=c(indexOfHours),labels=hours)+
    labs(title = chartTitle, x = "Time of Day", y = "Cumulative Motion Index", colour="")+
    geom_abline(intercept=as.numeric(bestFitAwake$Intercept),slope=as.numeric(bestFitAwake$Slope))
  

  return(ggplot(workingData,aes(x=Minutes,y=MotionIndex,group=Day, color = Group)) + 
           geom_line() +
           geom_text_repel(aes(label=Label)) +
           theme(legend.position = "none") +
           scale_x_continuous(breaks=c(indexOfHours),labels=hours)+
...
printFiles <- function() {
  ...
  pdf("/Users/samzimmer/Desktop/pigs/Visualizations/smoothMotionIndex.pdf")
  print("Files loaded")
