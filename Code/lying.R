...
makeGraph <- function(theFile) {
  ...
  if (difference<0) {
    ...
    fillDayLying <-c(rep(0,times=length(fillDayTime)))
    fillDay <<- data.frame(Time=fillDayTime,Lying=fillDayLying)
  ...
  workingData$Lying <- ave(workingData$Lying,workingData$Day, FUN=cumsum)
  #add group row
  ...
  return(ggplot(workingData,aes(x=Minutes,y=Lying,group=Day, color = Group)) + 
           geom_line() +
           geom_text_repel(aes(label=Label)) +
           theme(legend.position = "none") +
           scale_x_continuous(breaks=c(indexOfHours),labels=hours)+
           labs(title = chartTitle, x = "Time of Day", y = "Cumulative Lying Time", colour="")
  )
  ggplot(workingData,aes(x=Minutes,y=Lying,group=Day, color = Group)) + 
    geom_line() +
    geom_text_repel(aes(label=Label)) +
    theme(legend.position = "none") +
    scale_x_continuous(breaks=c(indexOfHours),labels=hours)+
...
printFiles <- function() {
  ...
  pdf("/Users/samzimmer/Desktop/pigs/Visualizations/lying.pdf")
  print("Files loaded")
