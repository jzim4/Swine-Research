...
makeGraph <- function(theFile) {
  ...
  colnames(myData)[colnames(myData) == "Lying Bouts"] <- "LyingBouts"
  
  
  #CONVERTING UNITS TO MAKE SENSE
  #date and time columns
  myData$Time <- as.numeric(myData$Time)
  myData$LyingBouts <- as.numeric(myData$LyingBouts)
  myData <- myData[c(2:7)]
  
  #ADJUSTING FOR START TIME OF 10AM
  workingData <- myData[,c(1,6)]
  ...
  if (difference<0) {
    ...
    fillDayLyingBouts <-c(rep(0,times=length(fillDayTime)))
    fillDay <<- data.frame(Time=fillDayTime,LyingBouts=fillDayLyingBouts)
  ...
  workingData$LyingBouts <- ave(workingData$LyingBouts,workingData$Day, FUN=cumsum)
  #add group row
  ...
  return(ggplot(workingData,aes(x=Minutes,y=LyingBouts,group=Day, color = Group)) + 
           geom_line() +
           geom_text_repel(aes(label=Label)) +
           theme(legend.position = "none") +
           scale_x_continuous(breaks=c(indexOfHours),labels=hours)+
           labs(title = chartTitle, x = "Time of Day", y = "Cumulative Lying Bouts", colour="")
  )
  ggplot(workingData,aes(x=Minutes,y=LyingBouts,group=Day, color = Group)) + 
    geom_line() +
    geom_text_repel(aes(label=Label)) +
    theme(legend.position = "none") +
    scale_x_continuous(breaks=c(indexOfHours),labels=hours)+
...
printFiles <- function() {
  ...
  pdf("/Users/samzimmer/Desktop/pigs/Visualizations/lyingBouts.pdf")
  print("Files loaded")
